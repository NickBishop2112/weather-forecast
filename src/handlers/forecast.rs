...
pub async fn get_weather(client: web::Data<Arc<dyn HttpClient>>,config_provider: web::Data<Arc<dyn ConfigProvider>>) -> Result<HttpResponse,Error> {
...
#[cfg(test)]
mod tests {
    use crate::{config::settings::{AppConfig, MockConfigProvider}, services::http_client::MockHttpClient, Error};
    ...
    async fn test_get_weather_failure() {
        ...
        // Assert
        assert_eq!(response.unwrap().status(), actix_web::http::StatusCode::INTERNAL_SERVER_ERROR);
    }
}